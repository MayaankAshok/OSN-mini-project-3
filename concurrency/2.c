...
int main(void){
	...
	srand(time(NULL));
    CafeInfo cInfo;
    scanf("%d %d %d %d", &cInfo.N, &cInfo.K, &cInfo.F, &cInfo.T);

    cInfo.start_time = malloc(cInfo.N* sizeof(int) );
    cInfo.end_time = malloc(cInfo.N* sizeof(int) );
    for (int i = 0; i < cInfo.N; i++)
    {
        scanf("%d %d", &cInfo.start_time[i], &cInfo.end_time[i]);
    }

    cInfo.flavours = malloc(cInfo.F* sizeof(char*));
    cInfo.flav_avb = malloc(cInfo.F* sizeof(int));
    for (int i = 0; i < cInfo.F; i++)
    {
        cInfo.flavours= malloc(MAX_STR_LEN*sizeof(char));
        scanf("%s %d", cInfo.flavours[i], &cInfo.flav_avb[i]);
    }
    cInfo.toppings = malloc(cInfo.T* sizeof(char*));
    cInfo.top_avb = malloc(cInfo.T* sizeof(int));
    for (int i = 0; i < cInfo.T; i++)
    {
        cInfo.toppings= malloc(MAX_STR_LEN*sizeof(char));
        scanf("%s %d", cInfo.toppings[i], &cInfo.top_avb[i]);
    }
